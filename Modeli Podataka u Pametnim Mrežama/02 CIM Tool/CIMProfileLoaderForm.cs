        ...
        private void LoadCIMRDFSFile()
        {
            ...
            try
            {
                ...
                using (FileStream fs = File.Open(textBoxCIMProfile.Text, FileMode.Open))
                {
                    ...
                    // upisi vrednosti u text boxove
                    brojKlasaTb.Text = "Klasa: " + Program.brojKlasa.ToString();
                    brojAtributaTb.Text = "Atributa: " + Program.brojAtributa.ToString();
                    Program.brojKlasa = Program.brojAtributa = 0; // resetovanje brojaca
                }
            }
            catch (Exception e)
            {
